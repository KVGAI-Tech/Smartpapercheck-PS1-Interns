...
const FilterDropdown = ({ label, options, value, onChange }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="text-sm text-gray-700">{label}: <span className="font-medium">{value}</span></span>
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${bgColor} ${textColor}`}>
      {icon}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-medium text-gray-600">{requests.length} Requests</span>
                  ...
                  {requests.map((request, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{request.examTitle}</div>
                        <div className="text-xs text-gray-500">{request.examType}</div>
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-right">
                        <Link 
                          to={`/student/evaluations/${course.id}/exam/${request.examId}`}
...
const StudentRecheckRequests = () => {
  ...
  const semesterOptions = ['All Semesters', 'Spring 2024', 'Fall 2023', 'Spring 2023'];
  ...
  const totalRequests = filteredCourses.reduce((total, course) => total + course.requests.length, 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">View and track your recheck requests by course and exam</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between bg-white p-4 rounded-xl shadow-sm">
        <div className="text-sm text-gray-500">
          Showing <span className="font-medium text-gray-900">{totalRequests}</span> requests across <span className="font-medium text-gray-900">{filteredCourses.length}</span> courses
        </div>
      ...
      {filteredCourses.length > 0 ? (
        <div>
          {filteredCourses.map((course, index) => (
            <CourseSection key={index} course={course} requests={course.requests} />
          ))}
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No Recheck Requests Found</h3>
          <p className="text-gray-500 max-w-md mx-auto">
            {searchQuery || selectedStatus !== 'All Status' || selectedSemester !== 'All Semesters' 

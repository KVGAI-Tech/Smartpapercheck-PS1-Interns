...
const highlights = [
  {
    icon: <HiOutlineCurrencyRupee className="w-8 h-8 text-teal-500" />,
  ...
  {
    icon: <HiOutlineClock className="w-8 h-8 text-blue-500" />,
    title: "Time Savings",
    ...
    <section className="w-full py-12 bg-gradient-to-b from-white to-blue-50">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6">
        {highlights.map((h, i) => (
          <div
            key={i}
            className="bg-white rounded-2xl shadow-lg border border-blue-100 p-6 flex flex-col gap-3 hover:shadow-2xl transition"
          >
            <div className="flex items-center gap-2 mb-2">
              {h.icon}
              <span className="text-lg font-bold text-gray-900">{h.title}</span>
            </div>
            <div className="text-xl font-bold text-blue-800">{h.main}</div>
            {h.sub && <div className="text-base font-semibold text-blue-600">{h.sub}</div>}

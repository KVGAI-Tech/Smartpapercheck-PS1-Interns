...
const FloatingShapes = () => {
  const shapes = [
    { 
      type: 'rect', 
      className: 'w-8 h-8 rounded-lg bg-blue-400/30 backdrop-blur-sm border border-blue-400/50',
      animate: {
        y: [-10, 10, -10],
        rotate: [0, 10, 0],
        scale: [1, 1.05, 1]
    ...
    { 
      type: 'circle', 
      className: 'w-10 h-10 rounded-full bg-purple-400/30 backdrop-blur-sm border border-purple-400/50',
      animate: {
        y: [10, -10, 10],
        x: [-5, 5, -5],
        scale: [1, 1.1, 1]
    ...
    { 
      type: 'triangle', 
      className: 'w-0 h-0 border-l-[12px] border-r-[12px] border-b-[20px] border-l-transparent border-r-transparent border-b-green-400/50',
      animate: {
        y: [0, -15, 0],
        rotate: [0, -10, 0],
        scale: [1, 0.9, 1]
    ...
    { 
      type: 'rect', 
      className: 'w-6 h-14 rounded-lg bg-indigo-400/30 backdrop-blur-sm border border-indigo-400/50',
      animate: {
        x: [-8, 8, -8],
        rotate: [0, -5, 0],
        scale: [1, 1.08, 1]
...
const RoleSelector = ({ selectedRole, onRoleSelect }) => {
  ...
  const roles = [
    { 
      ...
      description: 'Create exams, manage courses, and evaluate student performance',
      icon: (
  ...
  return (
    <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl p-8 w-full max-w-md mx-auto border border-gray-200/50">
      <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2 text-center">Choose Your Role</h2>
      <p className="text-gray-600 text-center mb-8">Select the role that best describes you</p>
            ...
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center
              ...
              {selectedRole === role.id && (
                <motion.div 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className={`w-3 h-3 rounded-full ${role.textColor.replace('text', 'bg')}`}
                />
              )}
            </div>
          </motion.button>
...
const RoleAuth = () => {
  ...
  const renderLoginForm = () => {
    switch (selectedRole) {
      case 'professor':
        return <ProfessorLogin onBack={handleBackToRoles} onLoginSuccess={handleLoginSuccess} />;
      case 'student':
        return <StudentLogin onBack={handleBackToRoles} onLoginSuccess={handleLoginSuccess} />;
      case 'ta':
        return <TALogin onBack={handleBackToRoles} onLoginSuccess={handleLoginSuccess} />;
      default:
        return null;
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 py-10 relative">
      ...
      <div className="w-full max-w-4xl relative z-10">
        <div className="text-center mb-10">
          ...
          >
            Welcome to <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">Smart QnA</span>
          </motion.h1>
          ...
          >
            AI-powered script evaluation for universities and educational institutions
          </motion.p>
        </div>
